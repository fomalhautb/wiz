...
impl Context {
    ...
    pub fn op_view_2d(&self, a: &Tensor, ne0: i32, ne1: i32, nb1: usize, offset: usize) -> Tensor {
        let tensor = unsafe {
    ...
    pub fn op_alibi(&self, a: &Tensor, npast: i32, ndims: i32, max_bias: f32) -> Tensor {
        let tensor = unsafe {
            ggml_raw::ggml_alibi(self.ptr.as_ptr(), a.ptr.as_ptr(), npast, ndims, max_bias)
        };
        self.new_tensor_raw(tensor)
    }
...
impl Tensor {
    ...
    fn with_alive_ctx<U>(&self, f: impl FnOnce() -> U) -> U {
...
impl Debug for Tensor {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        self.with_alive_ctx(|| return unsafe { (*self.ptr.as_ptr()).fmt(f) })
    }
}
